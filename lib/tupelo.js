...
class TupeloClient {
  ...
  exportChainTree(chainId) {
    return promiseAroundRpcCallback((clbk) => {
      this.rpc.exportChainTree({ creds: this.walletCreds, chainId: chainId }, clbk);
    });
  }
  /**
   * @typedef {Object} ExportChainTreeResponse
   * @property {Object} chainTree - The serialized chain tree
   * @property {string[]} chainTree.dag - base58 encoded string array representing the chain tree dag nodes
   * @property {Map<String, Object} chainTree.signatures - Map of serialized signatures
   */
