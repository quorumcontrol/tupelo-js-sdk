...
const walletCredMetadata = (walletCreds) => {
...
const connectionCredentials = (secure) => {
  if (secure) {
 ...
 */
class TupeloClient {
  constructor(walletServer, walletCreds, encrypt = true) {
    /**
    ...
    this.credentials = walletCredMetadata(walletCreds);
    /**
    ...
    this.rpc = new tupeloPackage
      .WalletRPCService(walletServer, connectionCredentials(encrypt));
  }
   ...
   * @property {string} path - Path to ipld initialized configuration
...
/**
 * Open a secure connection to a Tupelo wallet managed by a remote Tupelo RPC
 * server.
 *
 ...
 */
function connectTLS(walletServer, walletCreds) {
  return new TupeloClient(walletServer, walletCreds, true);
}

/**
 * Open an insecure connection to a Tupelo wallet managed by a remote Tupelo RPC
 * server. This function should only be used in development as it transmits
 * wallet credentials without any encryption.
 *
 * @param {string} walletServer - "host:port" string of the RPC wallet server
 * @param {Object} walletCreds - Credentials for the connecting wallet
 * @param {string} walletCreds.walletName - Wallet name
 ...
 * @return {TupeloClient} Tupelo client connection.
 */
function connectInsecure(walletServer, walletCreds) {
  return new TupeloClient(walletServer, walletCreds, false);
}

exports.TupeloClient = TupeloClient;
exports.connectTLS = connectTLS;
exports.connectInsecure = connectInsecure;
